...
with lib;
let
  cfg = config.vinlabs.llama-cpp;
in {
  options.vinlabs.llama-cpp.enable = mkEnableOption "llama-cpp local LLM";
  ...
  config = mkIf cfg.enable {
    services.llama-swap = {
      ...
      in {
        ...
        models = {
          "gemma-4-E4B" = { cmd = "${llama-server} -m ${gemma-4-E4B} ${stand-arg} --ctx-size 32768 --jinja"; };
          "gemma-4-12B" = {
            cmd = "${llama-server} ${stand-arg} -m ${gemma-4-12B-model} -mmproj ${gemma-4-12B-mmproj} -ngl 99 --ctx-size 8192 --jinja -fa on";
            filter = {
              stripParams = "temperature, top_p, min_p, top_k, repeat_penalty";
              setParams = { temperature = 0.6; top_p = 0.9; top_k = 64; min_p = 0.05; repeat_penalty = 1.1; };
            };
          };
          "GPTOSS-20B" = { cmd = "${llama-server} -m ${gptoss-20b} ${stand-arg}"; name = "gpt-oss 20B"; };
          "Qwen3.6-35B-A3B" = { cmd = "${llama-server} -m ${qwen36-35B-A3B} ${stand-arg} --ctx-size 65536 --jinja"; };
        };
      };
    };
  };
